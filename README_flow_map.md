...
- **Note on Usage:** The `data/collected_igod_urls.txt` file is generated by this pre-stage. However, this list of general government URLs is not automatically consumed by the subsequent event-specific scraping scripts (like `event_scraper_v1.py`) in the documented pipeline. Its primary use is likely for manual review to identify new sites, which might then be manually incorporated into the target lists of other scripts or used for other purposes. The script `curate_urls.py` processes a different file (`data/government_sites.txt`).
