...
[ Stage 2: Event Classification (Speculative) ]
  ...
  Output: data/extracted_events_v4_llm_classified.json (Refined list of event candidates)
            (Contains more qualified "Decided URLs")
     |
     V
[ Stage 3: Detailed Event Scraping & Enrichment ]
  ...
  Process:
    1. Fetch HTML of the event *detail page* (`source_url`).
       (`fetch_html()`)
